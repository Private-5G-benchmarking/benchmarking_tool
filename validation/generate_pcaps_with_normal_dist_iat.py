...
def generate_packet_stream(packet_count, payload_size, mu, sigma, dstIP, dstPort):
    ...
    normally_distributed_delays = np.random.normal(mu, sigma, packet_count)
    for i in range(packet_count):
        packet = generate_udp_packet(dstIP, dstPort, payload_size)
        packet.time = previous_time + normally_distributed_delays[i]
...
def main():
    ...
    packet_stream = generate_packet_stream(
        packet_count, payload_size, mu, sigma, dstIP, dstPort
    )
